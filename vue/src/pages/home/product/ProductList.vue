...
<div>
    ...
    <el-table stripe :data="employee" style="border: 1px solid #ebebeb;border-radius: 3px;margin-top: 10px;">
<!--      <el-table-column prop="userid" label="用户ID" width="100">-->
<!--        <template slot-scope="scope">-->
<!--          &lt;!&ndash; el-checkbox 多选框，checked 绑定是否选中，click 绑定点击事件 &ndash;&gt;-->
...
<!--      </el-table-column>-->
      ...
      <el-table-column prop="birthData" label="生日" width="200"></el-table-column>
      <el-table-column  prop="gender" label="性别" width="200"></el-table-column>
      <el-table-column prop="mileage" label="里程" width="200"></el-table-column>
...
export default {
  ...
  created(){
    const that = this
    axios.get('http://localhost:8088/employee/findAll/1/20').then(function (resp) {
      console.log(resp)
        that.total=resp.data.totalElements
      that.employee=resp.data.content
  ...
  methods: {
      page(currentPage){
          const that = this
          axios.get('http://localhost:8088/employee/findAll/'+currentPage+'/20').then(function (resp) {
              console.log(resp)
              that.total=resp.data.totalElements
              that.employee=resp.data.content
